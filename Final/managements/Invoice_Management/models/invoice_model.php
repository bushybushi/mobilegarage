...
if (!isset($pdo) || !$pdo) {
    error_log("Database connection not available in invoice_model.php");
...
class InvoiceManagement {
    ...
    public function __construct() {
        $this->conn = require dirname(__DIR__) . '/config/db_connection.php';
    ...
    public function ViewSingle($id) {
        try {
            ...
            if (!empty($invoice['parts'])) {
                ...
                if ($supplierData) {
                    $invoice['SupplierID'] = $supplierData['SupplierID'] ?? null;
                    $invoice['SupplierName'] = $supplierData['SupplierName'] ?? null;
                    $invoice['SupplierPhone'] = $supplierData['SupplierPhone'] ?? null;
                    $invoice['SupplierEmail'] = $supplierData['SupplierEmail'] ?? null;
                }
            }
    ...
    public function getTotalInvoices() {
        try {
            ...
            error_log("Error in getTotalInvoices: " . $e->getMessage());
            return 0;
        }
    }

    /**
     * Get invoices for printing with pagination and sorting
     * 
     ...
     */
    public function getPrintInvoices($page, $perPage, $sort) {
        try {
            ...
            error_log("Error in getPrintInvoices: " . $e->getMessage());
            return false;
        }
    }

    /**
     * Get selected invoices by their IDs
     * 
     * @param array $ids Array of invoice IDs
     * @return array Array of selected invoices
     */
    public function getSelectedInvoices($ids) {
        try {
            if (empty($ids)) {
                return [];
            }
            ...
            error_log("Error in getSelectedInvoices: " . $e->getMessage());
            return [];
        }
    }
