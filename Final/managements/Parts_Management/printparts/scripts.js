...
function loadPrintModalPage() {
    $.ajax({
        ...
        success: function(response) {
            try {
                // Find the tbody element in the print modal
                const tbody = $('#printModal').find('tbody');
                if (tbody.length) {
                    ...
                    const visibleRows = tbody.find('tr:visible');
                    const visibleCheckboxes = visibleRows.find('.print-part-select');
                    const visibleChecked = visibleCheckboxes.filter(':checked');
                    $('#select-all-visible').prop('checked', visibleCheckboxes.length > 0 && visibleCheckboxes.length === visibleChecked.length);
                    ...
                    console.error('Table body not found in print modal');
                }
            } catch (error) {
...
// Function to update the print modal pagination UI
function updatePrintPagination() {
    const paginationContainer = $('#printModalPagination');
    ...
    paginationContainer.append(`
        <li class="page-item ${currentPrintPage <= 1 ? 'disabled' : ''}">
            <a class="page-link" href="#" data-page="${currentPrintPage - 1}">
...
function showCurrentPageItems() {
    const rows = $('#printPartsTable tbody tr');
...
$(document).on('click', '#printModalPagination .page-link', function(e) {
    e.preventDefault();
    const page = $(this).data('page');
    if (page && page >= 1 && page <= totalPrintPages) {
