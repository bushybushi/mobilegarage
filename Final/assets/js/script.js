...
function highlightActiveMenuItem() {
  ...
  const menuItems = document.querySelectorAll('.pc-sidebar .pc-navbar a');
  ...
  menuItems.forEach(item => {
    ...
    const normalizedCurrentPath = currentPath.replace(/^\/MobileGarage3\//, '');
    ...
    if (normalizedCurrentPath === normalizedHref) {
      ...
      // If the menu item is in a submenu, show the parent menu
      const parentSubmenu = item.closest('.pc-submenu');
      if (parentSubmenu) {
        parentSubmenu.style.display = 'block';
        const parentMenuItem = parentSubmenu.closest('.pc-item');
        if (parentMenuItem) {
