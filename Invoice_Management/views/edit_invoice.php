...
<div class="form-container">
    ...
    <form id="invoiceForm" action="../controllers/update_invoice_controller.php?id=<?php echo htmlspecialchars($id); ?>" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label for="invoicePhoto">Upload New Invoice Photo</label>
            ...
            <small class="form-text text-muted">Supported formats: JPG, PNG, GIF. Max size: 5MB. Leave empty to keep current photo.</small>
        </div>
        ...
        <?php endif; ?>
                        ...
                        <div class="part-actions">
                            ...
                            <button type="button" class="btn btn-danger delete-part-btn">
...
label[for="invoicePhoto"]:after,
label[for="photoPreview"]:after {
    content: "" !important;
}
</style>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // Clear supplier contact info when supplier name is changed
        $("#supplier").on('input', function() {
            if ($(this).val() !== originalSupplierData.name) {
                $("#supplierID").val('');
                $("#supplierPhone").val('').attr('placeholder', 'Required for new supplier');
                $("#supplierEmail").val('').attr('placeholder', 'Required for new supplier');
                isNewSupplier = true;
    ...
    // Update existing delete part handler
    document.addEventListener('click', function(e) {
        if (e.target.closest('.delete-part-btn')) {
            const partItem = e.target.closest('.part-item');
            const editBtn = partItem.querySelector('.edit-part-btn');
            const partId = editBtn.dataset.partId;
            const partDesc = editBtn.dataset.partDesc;

            // Check if this is the last part
            if ($('.part-item').length === 1) {
                alert('You cannot delete the last part. At least one part is required.');
                return;
            }

            if (confirm(`Are you sure you want to delete "${partDesc}" from this invoice?`)) {
                // Only ask about database deletion if it's not a temporary part
                if (!partId.startsWith('temp_')) {
                    if (confirm('Do you want to permanently delete this part?')) {
                        ...
                        $.ajax({
                            ...
                            success: function(result) {
                                if (result.success) {
                                    ...
                                    showMessage('Error: ' + (result.message || 'Failed to delete part'), 'error');
                                }
                            },
                            error: function(xhr, status, error) {
                                console.error('Delete part error:', error);
                                console.error('Server response:', xhr.responseText);
                                showMessage('Error deleting part: ' + error, 'error');
                            }
                        });
                    }
                } else {
                    // For temporary parts, just remove from invoice
                    partItem.remove();
                    formChanged = true;
                    showMessage('Part removed from invoice', 'success');
                }
            }
        }
    });
        ...
        function openPartModal(isNew = false) {
            if (isNew) {
                ...
                $('#partDesc').val('');
                $('#piecesPurch').val('');  // Removed default value
                $('#pricePerPiece').val('');
                $('#priceBulk').val('');
                $('#sellingPrice').val('');
